# Handler File: llm_api_handler.py
...
# =======================
# Model Limits and Token Encoder
# =======================
...
class LLMAPIHandler:
    """
    Unified handler for OpenAI and Anthropic API interactions with improved error handling,
    ...
    def __init__(self, request_timeout: float = 30.0, custom_rate_limits: Optional[Dict[str, Dict[str, int]]] = None):
        ...
        if not openai_api_key or not anthropic_api_key:
            raise EnvironmentError("Required API keys (OPENAI_API_KEY, ANTHROPIC_API_KEY) not found in environment variables")
    ...
    async def _async_process_regular(self, 
        ...
        try:
            ...
            # Increment failed_requests is handled in the decorator
            raise
    ...
    async def process(self,
        ...
        Returns:
            Single response, list of responses, or BatchResult containing all responses
        """
        start_time = time.perf_counter()
        ...
        try:
            ...
            if mode == "openai_batch" and not model.startswith('gpt-'):
                raise ValueError("OpenAI batch mode is only supported for OpenAI models.")
            ...
            if mode == "openai_batch":
                ...
                for prompt, result in zip(prompts, batch_results):
                    if isinstance(result, Exception):
                        ...
                        logger.error(f"Prompt '{prompt}' failed with error: {str(result)}")
                    else:
    ...
    async def _process_openai_batch(self, requests: List[Dict[str, Any]], response_format: Optional[Type[T]],
                                    output_dir: Optional[str], update_interval: int, original_prompts: List[str]) -> List[Any]:
        ...
        while True:
            ...
            if batch.status == "completed":
                ...
                with open(metadata_file_path, 'w') as f:
                    json.dump(job_metadata, f, indent=2)
                return [None] * len(requests)  # Return list of Nones indicating failure
...
async def run_llmapihandler_tests():
    ...
    try:
        ...
        # Test 3: Async batch mode for Anthropic
        logger.info("\n=== Testing async batch mode for Anthropic ===")
        batch_prompts = [
        ...
        logger.info(f"Async batch results: {batch_result}")
        success_count = sum(1 for res in batch_result if res is not None)
        failure_count = len(batch_result) - success_count
        logger.info(f"Async batch successes: {success_count}")
        logger.info(f"Async batch failures: {failure_count}")
        
        # Test 4: OpenAI batch mode
        logger.info("\n=== Testing OpenAI batch mode ===")
        openai_batch_prompts = [
            "Translate 'Hello' to Spanish.",
            "Translate 'Good morning' to French.",
            "Translate 'Thank you' to German."
        ]
        openai_batch_result = await handler.process(
            prompts=openai_batch_prompts,
            model="gpt-4o-mini",
            mode="openai_batch",
            output_dir="test_outputs_openai"
        )
        logger.info(f"OpenAI batch results: {openai_batch_result}")
